...
public class Arm implements AutoCloseable {
  ...
  private double m_armSetpointDegrees = 180;
  ...
  public void reachSetpoint() {
    var pidOutput =
        m_controller.calculate(
            m_encoder.getDistance(), Units.degreesToRadians(135));
    m_motor.setVoltage(pidOutput);
  }

  public void reachSetpointBasic() {
    if(m_encoder.getDistance() >= Units.degreesToRadians(135)) {
      m_motor.set(-0.5);
    }
    if(m_encoder.getDistance() <= Units.degreesToRadians(135)) {
      m_motor.set(0.5);  
  ...
  public void reachSetpoint2() {
    var pidOutput =
        m_controller.calculate(
            m_encoder.getDistance(), Units.degreesToRadians(45));
    m_motor.setVoltage(pidOutput);
  }
